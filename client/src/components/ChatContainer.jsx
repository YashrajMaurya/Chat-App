...
const ChatContainer = () => {
    ...
    return selectedUser ? (
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            {/* chat area  */}
            <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6 overflow-anchor-none'>
                {
                    messages.map((msg, ind) => (
                        <div className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`} key={ind}>
                            {
                                msg.image ? (
            ...
            {/* bottom area  */}
            <div className='absolute bottom-0 left-0 flex right-0 items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-[#8185B2]/10 px-3 rounded-full'>
                    <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSendMessage(e) : null} type="text" placeholder='Send a message' className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400 bg-transparent' />
